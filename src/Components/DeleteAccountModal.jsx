...
const DeleteAccountModal = ({ isOpen, onClose, onDelete }) => {
    const handleDeleteUser = async () => {
        try {
            onDelete(); // Callback to handle additional actions after deletion
        } catch (error) {
            console.log(error);
    ...
    return (
        <div className={`fixed top-0 left-0 right-0 z-50 flex items-center justify-center w-full h-full bg-black bg-opacity-50 ${isOpen ? '' : 'hidden'}`}>
            <div className="relative p-4 w-full max-w-md">
                <div className="relative bg-white rounded-lg shadow dark:bg-gray-700">
                    <button onClick={onClose} type="button" className="absolute top-3 end-2.5 text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center dark:hover:bg-gray-600 dark:hover:text-white">
